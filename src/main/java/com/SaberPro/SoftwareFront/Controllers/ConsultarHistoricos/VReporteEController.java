...
public class VReporteEController implements Initializable {
    private final int MIN_PUNTAJE = 0, MAX_PUNTAJE = 300; // Max puntaje es 300 para el rango slider
    //Filtros
    ...
    // Para el Label de vista actual
    @FXML private Label lblVistaActual;

    // Nombres de los ComboBoxes para el gráfico
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Listeners para actualizar el gráfico cuando cambian las selecciones de año o periodo
        ...
        txtField.textProperty().addListener((obs, oldVal, newVal) -> {
            if (esNumeroValido(newVal)) {
                ...
                if (valido) {
                    ...
                    txtField.setText(oldVal); // Revertir a valor anterior si es inválido
                }
            } else {
                txtField.setText(oldVal); // Revertir a valor anterior si no es número válido
            }
        });
    }
    @FXML
    ...
    private boolean esNumeroValido(String valor) {
        try {
            ...
            return num >= MIN_PUNTAJE && num <= MAX_PUNTAJE;
    ...
    private void aplicarFiltros() {
        ...
        // Asignar los valores del RangeSlider directamente a los filtros
    ...
    private void buscarReporteConFiltros(InputQueryDTO filtros) {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                mostrarAlerta("Error", "No se pudieron obtener los reportes: " + response.body());
            }
        } catch (JsonProcessingException e) {
            e.printStackTrace();
            mostrarAlerta("Error", "Error al procesar los filtros: " + e.getMessage());
        } catch (IOException | InterruptedException e) {
            e.printStackTrace();
            mostrarAlerta("Error", "Error al comunicarse con el servidor: " + e.getMessage());
        }
    }
    ...
    private void actualizarGrafico() {
        ...
        try {
            ...
            Map<String, Double> promediosGenerales = resultadosGenerales.stream()
                    .filter(r -> r.getPuntajeModulo() >= MIN_PUNTAJE && r.getPuntajeModulo() <= MAX_PUNTAJE)// Si r.getPuntajeModulo() es de tipo primitivo int,
                    // la condición '!= null' dará error.
                    // Asegúrate de que getPuntajeModulo() devuelva un Integer (wrapper).
                    // Si ReporteDTO.puntajeModulo es Integer, entonces esto está bien.
            ...
            for (String modulo : todosLosModulos) {
                ...
                if (reporteEstudianteModulo.isPresent()) { // Si getPuntajeModulo() devuelve int, no puede ser null
    ...
    private void mostrarAlerta(String titulo, String mensaje) {
        Alert alert = new Alert(Alert.AlertType.INFORMATION); // Cambiado a INFORMATION para mensajes no críticos
...
public class VReporteEController implements Initializable {
    ...
    // REMOVED: @FXML private VBox tipoModuloCheckboxes; // This VBox is no longer used for individual checkboxes
    // ADDED: ComboBox for module type
    @FXML private ComboBox<String> cmbTipoModulo;


    //Tabla
    // REMOVED: private final List<CheckBox> tipoModuloOptions = new ArrayList<>(); // No longer needed for checkboxes
    private ObservableList<ReporteDTO> listaReportes = FXCollections.observableArrayList();
    @FXML private TableView<ReporteDTO> tablaReportes;
    ...
    @FXML private ComboBox<Integer> comboAnioGrafico;
    @FXML private ComboBox<String> comboPeriodoGrafico;
    // @FXML private ComboBox<String> comboModuloGrafico; // REMOVED: Ya no se usa si siempre se muestran los 7 módulos
    @FXML private javafx.scene.chart.BarChart<String, Number> graficoBarras;
    @FXML private CategoryAxis ejeX;
    @FXML private NumberAxis ejeY;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        configurarComboBoxes();
        configurarTipoModuloOptions(); // Now configures the ComboBox
        configurarColumnasTabla();
